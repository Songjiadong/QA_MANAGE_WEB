...
(function ($, undefined) {
    ...
    // 全局对象，可扩展
    OBJREG = {
        CHINESE: "/[^\u4e00-\u9fa5]/",//majichao 20170323
        CHINESEXSTR: "^[\u4e00-\u9fa5_a-zA-Z0-9]+$",//majichao 20170323
        PRICE: "^(([1-9]\\d{0,9})|0)(\\.\\d{1,2})?$",
        EMAIL: "^[a-z0-9._%-]+@([a-z0-9-]+\\.)+[a-z]{2,4}$",
        NUMXSTR: "^[A-Za-z0-9\\._]+$",//majichao 20170323
        NUMSTR: "^\\d+$",
        CODE: "^[\\w\\d_-\\+\\.]*$",
        URL: "^(http|https|ftp)\\:\\/\\/[a-z0-9\\-\\.]+\\.[a-z]{2,3}(:[a-z0-9]*)?\\/?([a-z0-9\\-\\._\\?\\,\\'\\/\\\\\\+&amp;%\\$#\\=~])*$",
        TEL: "^(0\\d{2,3}-\\d{7,8})$",//majichao 20160517
        TELTWO: "^((0\\d{2,3}-\\d{7,8})(,)(0\\d{2,3}-\\d{7,8}))|((0\\d{2,3}-\\d{7,8}))$",//linan 20170508
        FAX: "^(0\\d{2,3}-\\d{7,8})$",//majichao 20160517
        MOBILE: "^(1[35847]\\d{9})$",//majichao 20160517
        QQ: "^[1-9][0-9]{4,9}$",//majichao 20160517
        GUID: "^[A-Z0-9]{8}-[A-Z0-9]{4}-[A-Z0-9]{4}-[A-Z0-9]{4}-[A-Z0-9]{12}$",//majichao 20170323
        XGUID: "^[A-Za-z0-9]{32}$",//majichao 20170323
        ZIPCODE: "^\\d{6}$",
        ...
        TAX: "^[A-Z0-9]{15}$|^[A-Z0-9]{18}$|^[A-Z0-9]{20}$",
        "prompt": {
            chinese: "请输入中文",//majichao 20170323
            chinesexstr: "请输入中文字母数组或下划线组成的字符串",//majichao 20170323
            price: "请输入正确金额",
            radio: "请选择一个选项",
            checkbox: "如果要继续，请选中此框",
            "select": "请选择列表中的一项",
            email: "请输入正确的电子邮件地址",//majichao 20160517
            url: "请输入正确的网站地址",
            tel: "请输入正确的电话号码",//majichao 20160517
            teltwo: "请输入正确的电话号码",//linan 20170508
            fax: "请输入正确的传真号码",//majichao 20160517
            mobile: "请输入正确的手机号码",//majichao 20160517
            qq: "请输入正确的QQ号码",//majichao 20160517
            guid: "请输入正确的ID",//majichao 20170323
            xguid: "请输入正确的ID",//majichao 20170323
            zipcode: "请输入正确的邮编",//majichao 20160517
            numxstr: "请输入字母、数字、“_”和“.”组成的字符串",//majichao 20170323
            numstr: "请输入数字",
            code: "请输入正确编码",
            date: "请输入日期",
            pattern: "内容格式不符合要求",
    ...
    $.html5Validate = (function () {
        ...
        return {
            ...
            isAllpass: function (elements, options) {
                ...
                if (elements.size && elements.size() == 1 && elements.get(0).tagName.toLowerCase() == params.formContainer) {//majichao 20160517
                    elements = elements.find(":input");
                } else if (elements.tagName && elements.tagName.toLowerCase() == params.formContainer) {//majichao 20160517
                    elements = $(elements).find(":input");
                }
                var self = this;
                var allpass = true
    ...
    $.fn.extend({
        ...
        html5Validate: function (callback, options) {
            var defaults = {
                ...
                //submit button
                submitID: "submit",
                //表单容器 默认为form 可选为 table|section|div
                formContainer: "form",
                //回调参数
                data: {},
                // 额外的其他验证
            ...
            //majichao 20160517
            if (params.formContainer == "form") {
                $(this).unbind().bind("submit", function (event) {
                ...
                var _this = this;
                $("#" + submitID).off("click").on("click", { CallbackData: callback_data }, function (event) {
                    var callback_data = event.data.CallbackData;
                    var elements = $(_this).find(":input");
                    //  例如type="email|"此时后面|需要去除
                    elements.each(function () {
                        var type = this.getAttribute("type") + "", typeReplaced = type.replace(/\W+$/, "");
                        if (type != typeReplaced) {
                    ...
                    if ($.html5Validate.isAllpass(elements, params) && params.validate() && $.isFunction(callback)) {
                        callback.call(_this, callback_data);
                    }
                    event.preventDefault();
                    return false;
                });
