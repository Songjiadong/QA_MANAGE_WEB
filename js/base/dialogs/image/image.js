...
//                }
        ...
        close: function ($img) {
            ...
            }).prev().on("click", { Path: $img.attr("src") }, function (event) {
                var _this = $(this);
                var path = event.data.Path;
                $.SimpleAjaxPostWithError("service/NewsService.asmx/DeleteTempFile",
                                          true,
                                          "{tempPath:'" + path + "'}",
                                          function (x, e) {
                                              ...
                                              var result = json.d;
                                              if (result == true) {
                                                  if (_this.parent().remove().hasClass("edui-image-upload-item")) {
                                                      //显示图片计数-1
                                                      Upload.showCount--;
                                                      Upload.updateView();
                                                  }
    ...
    var Upload = {
        ...
        uploadComplete: function(r){
            ...
            try{
                var json = eval('(' + r + ')');
                var url = json.acc.FilePath;
                var state = json.result;
                Base.callback(me.editor, me.dialog, url, state);
